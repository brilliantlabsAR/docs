...
| `Fill(color)` **class**                                       | Creates a color fill object covering the entire display which can be passed to `display.show()`. `color` can be any color from the available color palette.
...
| `VLine( x, y, height, color, thickness=1)` **class**          | Creates a vertical line object at position `x, y` as long as `height` which can be passed to `display.show()`. `color` can be any color from the available color palette, and `thickness` can optionally be provided to override the default line thickness in pixels.
| `HLine( x, y, width, color, thickness=1)` **class**           | Creates an horizontal line object at position `x, y` as long as `width` which can be passed to `display.show()`. `color` can be any color from the available color palette, and `thickness` can optionally be provided to override the default line thickness in pixels.
| 
...
| `clear()` **function**                                        | Clear the display, set it all transparent/black

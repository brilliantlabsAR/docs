...
| Members | Description |
|:--------|:------------|
| `read(addr, data[])` **function** ❌            | Fill the bytearray `data[]` with the data read from the 16-bit address `addr`
| `write(addr, data[])`&nbsp;**function**&nbsp;❌ | Writes all bytes from a given bytearray `data[]` to the 16-bit address `addr`
